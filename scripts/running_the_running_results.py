...
def read_pgm(address):
    """
    read the map and get its settings
    """
...
def point_clear(map_array, map_options):
    """
    get a clear point that the robot can start or finish on.
    """
    # pixel world
    ...
    while found_one_black:
        ...
        # min(map_options['seen']): #clear and seen
        if map_array[randomPicked[1]][randomPicked[0]] <= 208:
            found_one_black = True
            continue

        for i in range(-radius, radius):
            for j in range(-radius, radius):
                # ==  min(map_options['seen']):
                if map_array[int(randomPicked[1]+i)][int(randomPicked[0]+j)] < 100:
                    found_one_black = True
...
def get_points(map_array, map_options):
    """
    create all the points for the robot (real,fake,goal)
    """
    ...
    while not points_off_map:
        ...
        fake_x, fake_y = radius * \
            np.cos(theta) + real_x, radius * np.sin(theta) + real_y

        fake_x_in = map_options['origin'][0] < fake_x < (
            map_options['origin'][0] + map_options['width'] * map_options['resolution'])
        fake_y_in = map_options['origin'][1] < fake_y < (
            map_options['origin'][1] + map_options['height'] * map_options['resolution'])
        real_x_in = map_options['origin'][0] < real_x < (
            map_options['origin'][0] + map_options['width'] * map_options['resolution'])
        real_y_in = map_options['origin'][1] < real_y < (
            map_options['origin'][1] + map_options['height'] * map_options['resolution'])
        goal_x_in = map_options['origin'][0] < goal_x < (
            map_options['origin'][0] + map_options['width'] * map_options['resolution'])
        goal_y_in = map_options['origin'][1] < goal_y < (
            map_options['origin'][1] + map_options['height'] * map_options['resolution'])
...
def create_dataPoints(name, map_array, map_options):
    """
    create database with points for the robot
    """
    if 'lab' in name:
        return
    if os.path.exists('Results/' + name + '_data_points.csv'):
        return
    dataPoints = DataFrame()
    for i in range(100):
        real, fake, goal = get_points(map_array, map_options)
        dataPoints = dataPoints.append(
            {'real': real, 'fake': fake, 'goal': goal}, ignore_index=True)
...
def from_path_str_to_position_list(text):
    """
    reduce a plan to array of points
    """
    try_text = text[1:-1]
