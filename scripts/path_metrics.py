...
class PathMetrics:
    ...
    def __init__(self, ns='/', k=100, real_start=None, global_origin=None, resolution=None):
        ...
        rospy.loginfo(
            'Started Metric calculation, with sample size {0} and in ns {1}'.format(k, ns))

        pf_topic = self.ns + 'particlecloud'
        msg = rospy.wait_for_message(pf_topic, PoseArray)
        possible_poses_array = msg.poses
        rospy.loginfo('Number of possible start locations {}'.format(
            len(possible_poses_array)))
    ...
    def get_metrics_to_goal(self, goal_location):
        ...
        odomPose = self.get_poseStamp_from_position(
            self.get_robot_location_odometry())
    ...
    def single_plan_analysis(self, start, goal, show=False):
        ...
        grid_indices = self.reduce_poses_resolution(
            steps_array, len(self.heat_map[0]), len(self.heat_map))
        ...
        self.counter_along_path.append(
            self.get_landmarks_along_path(grid_indices))
    ...
    def plan_by_grid(self, grid_indices):
        ...
        for i, point in enumerate(grid_indices):
            if i == 0:
                continue
            plan_grid.append(
                (previous_location[0]-point[0], previous_location[1]-point[1]))
    ...
    def get_plan_different_analysis(self):
        ...
        heat_map_array_nonZero = heat_map_array_nonZero[np.nonzero(
            heat_map_array_nonZero)]
        ...
        non_zero_percent = non_zero_count / \
            float(self.heat_map.shape[0]*self.heat_map.shape[1])
    ...
    def get_map(self):
        ...
        """
        self.local_map = rospy.wait_for_message(
            self.ns + 'move_base/local_costmap/costmap', OccupancyGrid)
    ...
    def get_landmarks_along_path(self, grid_indices):
        ...
        for i in range(0, len(grid_indices), steps):
            x, y = grid_indices[i]
            map_in_radius = np.array(
                self.global_map[x - radius: x + radius, y - radius: y + radius])
            counter.append(np.count_nonzero(
                map_in_radius > obstacle_threshold))
    ...
    def will_do_replan(self, start_point, goal):
        ...
        previous_point = (
            (np.array(start) - np.array(self.global_origin)) / self.resolution).astype(np.int)
        index_in_array = 0
        ...
        steps_till_replan_array=index_in_array
        success_percent= (len(self.grid_plan_odom_position) == index_in_array)

        if index_in_array >= len(self.plan_odom_position):
            index_in_array = len(self.plan_odom_position)-1
        if len(self.plan_odom_position)==0:
            return sqrt((goal_pose.x - start[0])**2 + (goal_pose.y - start[1])**2)
        real_pose_stuck = self.plan_odom_position[index_in_array].pose.position
        diff = (self.plan_odom_position[0].pose.position.x - real_pose_stuck.x,
                self.plan_odom_position[0].pose.position.y - real_pose_stuck.y)
        fake_pose = (start[0]-diff[0], start[1]-diff[1])


        distance_goal = sqrt(
            (goal_pose.x - fake_pose[0])**2 + (goal_pose.y - fake_pose[1])**2)

        self.steps_replan_array.append(steps_till_replan_array)
        self.notNeed_to_replan_array.append(1 if success_percent else 0)
    ...
    def get_dead_reckoing(self):
        ...
        # (float(non_zeros) / float(len(self.counter_replan_paths))) * 100
        percent = np.mean(self.notNeed_to_replan_array)
...
if __name__ == '__main__':
    ...
    path_metrics = PathMetrics(
        real_start=real, global_origin=global_origin, resolution=res, k=100)
